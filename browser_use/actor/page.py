...
class Page:
	...
	def __init__(
		self, browser_session: 'BrowserSession', target_id: str, session_id: str | None = None, llm: object = None
	):
		...
		self._llm = llm  # Kept for API compat, LLM support removed
	...
	async def get_element_by_prompt(self, prompt: str, llm: object = None) -> 'Element | None':
		"""Get an element by a prompt. LLM support has been removed from browser-use."""
		raise RuntimeError('LLM support has been removed from browser-use. get_element_by_prompt requires an LLM.')

	async def must_get_element_by_prompt(self, prompt: str, llm: object = None) -> 'Element':
	...
	async def extract_content(self, prompt: str, structured_output: type[T], llm: object = None) -> T:
		"""Extract structured content from the current page. LLM support has been removed from browser-use."""
		raise RuntimeError('LLM support has been removed from browser-use. extract_content requires an LLM.')
