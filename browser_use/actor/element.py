...
class Element:
	...
	async def click(
		...
		try:
			...
			# Import the Mouse class to use realistic movement
			from browser_use.actor.mouse import Mouse

			# Create mouse instance for realistic movement
			mouse = Mouse(self._browser_session, self._session_id)

			# Perform the click using CDP with realistic human-like movement
			try:
				# Use realistic mouse movement to the element
				await mouse.move_to(int(center_x), int(center_y))
				...
				try:
					...
					# Human-like pause between press and release
					hold_duration = random.uniform(0.06, 0.18)
